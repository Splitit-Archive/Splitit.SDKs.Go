...
// LogoutApi service interface
type LogoutApiService interface {
	LogoutPost(ctx _context.Context, request map[string]interface{}) (LogoutResponse, *_nethttp.Response, error)
}

// LogoutApi service implementation
type implLogoutApiService struct {
	*APIClient
}
...
*/
func (a implLogoutApiService) LogoutPost(ctx _context.Context, request map[string]interface{}) (LogoutResponse, *_nethttp.Response, error) {
	var (
	...
	localVarPath := a.cfg.BasePath + "/api/logout"
	...
	if localVarReturnValue.ResponseHeader.Succeeded != true {
		if len(localVarReturnValue.ResponseHeader.Errors) > 0 {
			for _, apiErr := range localVarReturnValue.ResponseHeader.Errors {
				...
				return a.LogoutPost(ctx , request)
			}
			newErr := GenericOpenAPIError{
				body:  localVarBody,
